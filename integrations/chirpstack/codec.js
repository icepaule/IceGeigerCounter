...
function decode(bytes){
  // v1 payload: 1+4+2+2+4+4+1+1+2 = 21 bytes
  if(!bytes||bytes.length<21)throw new Error('IceGeiger payload too short');
  if(bytes[0]!==1)throw new Error('Unsupported payload version');
  return {version:1,seq:u32(bytes,1),cpm:u16(bytes,5),usvh:u16(bytes,7)/1000,
    latitude:i32(bytes,9)/1e7,longitude:i32(bytes,13)/1e7,
    satellites:bytes[17],hdop:bytes[18]/10,battery_mv:u16(bytes,19)};
}
